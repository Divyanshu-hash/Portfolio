  ...
  return (
    <footer className="relative bottom-0 w-full h-[50vh] flex justify-between items-center flex-col px-4 py-8 bg-footer">
      ...
      <div className="w-full flex_center flex-col gap-2">
        <img src={config.logo as unknown as string} alt="LogoIcon" width={120} height={120} />
        <span>{config.title}</span>
      </div>

      <div className="grid grid-cols-3 lg:grid-cols-6 gap-x-20 gap-y-10 lg:gap-20">
        {config.links?.map((link: { url: string; label: string; icon: string }) => (
          <a
            key={link.label}
            href={link.url}
            title={link.label}
      ...
      <span>© Copyright 2023-{currentYear} DivyanshuGiri</span>
    </footer>
  );
}
