...
export function initAnimations() {
  ...
  if (!isInlineMobileMenuController) {
      HamMenuButton?.addEventListener('click', handleHamClick);
  }
  ...
  const handleThemeToggle = () => closeNav();
  
  if (!isInlineMobileMenuController) {
      themeToggleMobileBtn?.addEventListener('click', handleThemeToggle);
      themeToggleMobileProjectBtn?.addEventListener('click', handleThemeToggle);
  }

  const handlePageShow = () => closeNav();
  ...
  return () => {
      ...
      if (avatarImageElement) {
          avatarImageElement.removeEventListener('mousedown', onPressStart);
          avatarImageElement.removeEventListener('mouseup', onPressEnd);
          avatarImageElement.removeEventListener('mouseleave', onPressEnd);
          avatarImageElement.removeEventListener('touchstart', onPressStart);
          avatarImageElement.removeEventListener('touchend', onPressEnd);
          avatarImageElement.removeEventListener('touchcancel', onPressEnd);
      }

      if (!isInlineMobileMenuController) {
          HamMenuButton?.removeEventListener('click', handleHamClick);
          themeToggleMobileBtn?.removeEventListener('click', handleThemeToggle);
          themeToggleMobileProjectBtn?.removeEventListener('click', handleThemeToggle);
      }
